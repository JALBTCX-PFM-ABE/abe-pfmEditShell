...
*                                                                           *
*   Purpose:            Simple shell program to allow Optech's HydroFusion  *
*                       (or other programs) to shell pfmEdit3D without      *
*                       having to run pfmView.                              *
*                                                                           *
\***************************************************************************/
...
void pfmEditShell::usage ()
{
  ...
  fprintf (stderr, "\nOPTIONS:\n\n");
  fprintf (stderr, "\t-n = Start the editor in 'no invalid' data mode.\n");
  ...
  fprintf (stderr, "\t--force_auto_unload forces pfmEdit3D to unload edits on save/exit.\n");
...
pfmEditShell::pfmEditShell (int32_t *argc, char **argv)
{
  char               area_file[512];
  uint8_t            area_defined = NVFalse, kill_all;
  ...
  if (options.max_hsv_color < options.min_hsv_color)
    {
      uint16_t tmpu16 = options.max_hsv_color;
  ...
  if (pfm_handle < 0)
    {
      fprintf (stderr, "%s %s %s %d - %s - %s\n", progname, __FILE__, __FUNCTION__, __LINE__, misc.abe_share->open_args[0].list_path,
...
pfmEditShell::slotEditError (QProcess::ProcessError error)
{
  switch (error)
    ...
    case QProcess::FailedToStart:
      fprintf (stderr, "%s %s %s %d - Unable to start the edit process!\n", progname, __FILE__, __FUNCTION__, __LINE__);
      fflush (stderr);
      break;

    case QProcess::Crashed:
      fprintf (stderr, "%s %s %s %d - The edit process crashed!\n", progname, __FILE__, __FUNCTION__, __LINE__);
      fflush (stderr);
      break;

    case QProcess::Timedout:
      fprintf (stderr, "%s %s %s %d - The edit process timed out!\n", progname, __FILE__, __FUNCTION__, __LINE__);
      fflush (stderr);
      break;

    case QProcess::WriteError:
      fprintf (stderr, "%s %s %s %d - There was a write error to the edit process!\n", progname, __FILE__, __FUNCTION__, __LINE__);
      fflush (stderr);
      break;

    case QProcess::ReadError:
      fprintf (stderr, "%s %s %s %d - There was a read error from the edit process!\n", progname, __FILE__, __FUNCTION__, __LINE__);
      fflush (stderr);
      break;

    case QProcess::UnknownError:
      fprintf (stderr, "%s %s %s %d - The edit process died with an unknown error!\n", progname, __FILE__, __FUNCTION__, __LINE__);
      fflush (stderr);
      break;
    }
...
pfmEditShell::slotEditReadyReadStandardError ()
{
  ...
  if (!response.contains ("Destroyed while") && !response.contains ("failed to add paths") && !response.contains ("DrvShareLists"))
    {
      fprintf (stderr, "%s %s %s %d - %s\n", progname, __FILE__, __FUNCTION__, __LINE__, res);
      fflush (stderr);
    }
}
...
void 
pfmEditShell::editCommand (double *mx, double *my, int32_t count)
{
  pfmEditMod = 0;
  pfmEditFilt = 0;
  ...
  //  Save the count in case we get a regen signal from pfmEdit3D
  ...
  for (int32_t i = 0 ; i < count ; i++)
    {
      //  Save the points in case we get a regen signal from pfmEdit3D
  ...
  //  For rectangles we pass a count of zero to tell pfmEdit3D that it's a rectangle.
...
void 
pfmEditShell::slotQuit (int32_t status)
{
  //  If we got a regen signal (in case of czmilReprocess) from pfmEdit3D we need to just kick off the editor again.

  if (pfmEditMod >= 200)
    {
      editCommand (save_mx, save_my, save_count);
    }
  else
    {
